...
module.exports = {
  name: "help",
  description:
    "Lists all possible commands or information on a specific command.",
  usage: "[command name]",
  cooldown: 3,
  args: false,
  serverOnly: false,
  ...
  execute(message, args) {
    ...
    if (!args.length) {
      ...
      return message.author
        ...
        .then(() => {
          ...
          message.reply(
            "I've sent you a DM with all my commands. On Wisconsin!"
          );
        })
        .catch((error) => {
          console.error(
    ...
    if (!command) {
      return message.channel.send(
        "Uh oh, that doesn't appear to be a valid command!"
      );
    }
